...
## 테이블 생성 SQL
* ** create table business_times (id bigint generated by default as identity, close varchar(255), day varchar(255), open varchar(255), primary key (id));
* ** create table store (id bigint generated by default as identity, address varchar(255), description varchar(255), holidays binary(255), level bigint not null, name varchar(255), owner varchar(255), phone varchar(255), primary key (id))
* ** create table store_business_times (store_id bigint not null, business_times_id bigint not null)
* ** alter table store_business_times add constraint FK362oex789lon0cwlbkqq1nq06 foreign key (business_times_id) references business_times
* ** alter table store_business_times add constraint FK4nok2p8sxbnhc180b23f0gesj foreign key (store_id) references store
* **  JPA H2 서버 시작 하면 자동생성 서버 종료시 테이블 삭제됨
---
